...
def load_data(Data_path, Label_path, id_path):
    ...
    Y_val = Y_val.reshape([-1,128,128,2])
...
def overlay(slice1, slice2, index):
    ...
    plt.axis('off')
    plt.imshow(slice2, cmap=plt.cm.get_cmap('gnuplot', 2))
...
def overlayKey(slice2):
    fig, ax = plt.subplots()
    plt.axis('off')
    cax = ax.imshow(slice2, cmap=plt.cm.get_cmap('gnuplot', 2))
    ax.set_title('Ground Truth')
    cbar = fig.colorbar(cax, ticks = [0.25,0.75], orientation = 'horizontal')
    cbar.ax.set_xticklabels(['Background','Tumour'])
...
print("Dice scores using validation data: ", dice_function_loop_binary(Y_val_reshape, val_Y_pre))
print(report)
